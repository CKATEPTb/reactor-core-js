...
export class Mono<T> implements PipePublisher<T> {
    ...
    public switchIfEmpty(alternative: Publisher<T>): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                ...
                const operatorSub: Subscription = {
                    request(n: number) {
                        ...
                        demand = Math.min(demand + n, Number.MAX_SAFE_INTEGER);
                        if (altSub) altSub.request(n);
                        else primarySub.request(n);
                    },
                    unsubscribe() {
                        cancelled = true;
    ...
    /** On error from source, subscribes to replacement publisher instead. */
    public onErrorReturn(replacement: Publisher<T>): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                let primarySub: Subscription = { request() {}, unsubscribe() {} };
                let altSub: Subscription | null = null;
    ...
    /** If predicate returns true for the error, swallows it and completes empty; otherwise re-signals. */
    public onErrorContinue(predicate: (error: Error) => boolean): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                const sub = this.source.subscribe({
                    onSubscribe(_s) {},
    ...
    /** Runs fn before the first emitted value is forwarded downstream. */
    public doFirst(fn: () => void): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                let first = true;
                const sub = this.source.subscribe({
    ...
    /** Runs fn as a side-effect for each emitted value without affecting the stream. */
    public doOnNext(fn: (value: T) => void): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                const sub = this.source.subscribe({
                    onSubscribe(_s) {},
    ...
    /** Runs fn as a side-effect when an error is signalled. */
    public doOnError(fn: (error: Error) => void): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                const sub = this.source.subscribe({
                    onSubscribe(_s) {},
    ...
    /** Runs fn with the Subscription when onSubscribe is called. */
    public doOnSubscribe(fn: (subscription: Subscription) => void): Mono<T> {
        return new Mono<T>({
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                const sub = this.source.subscribe({
                    onSubscribe(_s) {},
    ...
    /** Delivers onNext/onError/onComplete via the given scheduler. */
    ...
    /** Performs the subscription on the given scheduler. */
            ...
            subscribe: (subscriber: Subscriber<T>): Subscription => {
                ...
                const proxy: Subscription = {
                    request(n: number) {
                        ...
                        if (sub) sub.request(n);
                        else pending = Math.min(pending + n, Number.MAX_SAFE_INTEGER);
                    },
                    unsubscribe() {
                        cancelled = true;
                        sub?.unsubscribe();
